...
function verificarSenha(senha, senhaHash) {
  ...
  const tentativa = crypto.createHash('sha256').update(salt + senha).digest('hex');
...
function authMiddleware(req, res, next) {
  ...
  const publicPaths = ['/api/auth/login', '/api/auth/setup', '/api/auth/status', '/login.html', '/setup.html'];
  if (publicPaths.some(p => req.path === p)) return next();

  // Assets estáticos (css, js, fontes, imagens)
  if (/\.(css|js|png|jpg|svg|ico|woff|woff2|ttf)$/.test(req.path)) return next();
...
function setupMiddleware(req, res, next) {
  ...
  if (!temUsuarios()) {
    if (req.path.startsWith('/api/')) {
      return res.status(403).json({ erro: 'Sistema não configurado', setup: true });
    }
