...
export default function AddressModal({ familia, onClose, onSaved }) {
  ...
  useEffect(() => {
    ...
    if (cidadeEstMatch) {
      ...
      const semCep = l3.replace(/\d{5}-?\d{3}/, '').replace(/[,\-\s]+$/, '').trim()
      setCidade(semCep)
    }
  ...
  function buildAddressLines() {
    ...
    if (complemento.trim()) linha2 += (linha2 ? ' - ' : '') + complemento.trim()

    const cepClean = cep.replace(/\D/g, '')
    const cepFormatted = cepClean.length === 8 ? cepClean.slice(0, 5) + '-' + cepClean.slice(5) : cepClean
    let linha3 = ''
    if (cidade.trim()) linha3 += cidade.trim()
    if (estado.trim()) linha3 += (linha3 ? '-' : '') + estado.trim().toUpperCase()
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (geo.sucesso) {
        ...
        showToast('Endereço salvo, mas não foi possível encontrar coordenadas. Você pode marcar manualmente no mapa.', 'warning')
      }
        ...
        <div className="modal-body">
          ...
          <div className="addr-row">
            <div className="addr-field addr-field-grow">
              ...
              <input type="text" value={rua} onChange={(e) => setRua(e.target.value)} placeholder="Rua Exemplo" />
            </div>
          ...
          <div className="addr-row">
            <div className="addr-field addr-field-grow">
              <label>Cidade</label>
              <input type="text" value={cidade} onChange={(e) => setCidade(e.target.value)} placeholder="São Paulo" />
            </div>
            <div className="addr-field addr-field-small">
              <label>UF</label>
              <input type="text" value={estado} onChange={(e) => setEstado(e.target.value.toUpperCase())} placeholder="SP" maxLength={2} />
            </div>
          </div>
