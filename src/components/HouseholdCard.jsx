    ...
    <div className="household-card" onClick={onSelect}>
      ...
      <div className="hc-address">
        ...
        {modified && !f.endereco_editado ? (
          <span
            className="modified-tag-small"
            title={`Alterado: ${modifiedFields.join(", ")}`}
          >
            ✏️
          </span>
        ) : null}
        <span className="hc-address-text">
          {f.endereco_linha1 || "Sem endereço"}
        </span>
        {f.endereco_linha2 && (
          <span className="hc-address-sub">{f.endereco_linha2}</span>
        )}
      </div>
      ...
      <div className="hc-members">
        {membros.slice(0, 6).map((m) => {
          ...
          return (
            ...
            >
              <span className={`hc-member-avatar ${avatarClass}`}>
                {initial}
              </span>
        ...
        {membros.length > 6 && (
          <span className="hc-member-more">+{membros.length - 6}</span>
        )}
      </div>
      ...
      <div className={`hc-details ${showActions ? "expanded" : ""}`}>
        <div className="hc-visit-info">
          {f.total_visitas > 0 ? (
            <>
              <span className="hc-visit-count">
                📒 {f.total_visitas} visita(s)
              </span>
              {f.ultima_visita && (
                <span className="hc-visit-date">
        ...
        {f.ultimaVisitaInfo && (
          <div className="hc-last-visit">
            ...
            {f.ultimaVisitaInfo.resultado && (
              <span
                className={`visita-resultado resultado-${f.ultimaVisitaInfo.resultado}`}
              >
                {RESULTADO_LABELS[f.ultimaVisitaInfo.resultado] ||
                  f.ultimaVisitaInfo.resultado}
              </span>
            )}
            <span className="hc-visit-visitor">
        ...
        <div className="hc-status-details">
          ...
          {f.interesse_retorno && f.interesse_retorno !== "nao_contatado" && (
            <span className="hc-tag">Interesse: {f.interesse_retorno}</span>
          )}
          {f.observacoes && (
            <span className="hc-tag hc-tag-obs" title={f.observacoes}>
              📝 Obs
            </span>
          )}
        </div>
