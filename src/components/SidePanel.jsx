...
import { formatDate, ROLE_LABELS, TIPO_LABELS, RESULTADO_LABELS, isModified, getModifiedFields } from '../lib/utils'
import AddressModal from './AddressModal'

export default function SidePanel({ open, familia, onClose, onRefresh, onStartPinMode }) {
  ...
  return (
    <div className={`side-panel ${open ? 'open' : ''}`}>
      ...
      <div className="familia-header">
        ...
        <div className="endereco endereco-clickable" onClick={() => setShowAddressModal(true)} title="Clique para editar endereço">
          {f.endereco_editado ? <span className="modified-tag">✏️ Editado</span> : null}
          {f.endereco_linha1 || ''}<br />
          {f.endereco_linha2 || ''}<br />
          {f.endereco_linha3 || ''}
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginTop: 8 }}>
          ...
          {modified && (
            <span className="modified-badge" title={`Campos alterados: ${modifiedFields.join(', ')}`}>
              ✏️ Modificado
            </span>
          )}
        </div>
      ...
      <div className="panel-section">
        ...
        <div className="status-controls">
          <div className="status-row">
            ...
            <select value={f.status} onChange={(e) => onStatusChange(e.target.value)}>
          ...
          {!isAtivo && (
            <div className="status-row">
              ...
              <select value={f.aceita_visitas} onChange={(e) => updateField('aceita_visitas', e.target.value)}>
                <option value="nao_contatado">🔘 Não Contatado</option>
          ...
          {isInativo && (
            <div className="status-row">
              ...
              <select value={f.interesse_retorno} onChange={(e) => updateField('interesse_retorno', e.target.value)}>
                <option value="nao_contatado">🔘 Não Contatado</option>
                <option value="sim">✅ Sim</option>
                <option value="nao">❌ Não</option>
        ...
        {(!f.latitude || !f.longitude) ? (
          <div className="coord-input-section">
            ...
            <div className="coord-input-row">
              <input value={coordInput} onChange={(e) => setCoordInput(e.target.value)} placeholder="-23.2237, -45.9009" />
              <button className="btn btn-primary btn-small" onClick={saveCoordinates}>Salvar</button>
            </div>
            <button className="btn btn-success btn-small" style={{ marginTop: 6, width: '100%' }} onClick={() => onStartPinMode(f.id)}>📍 Marcar no Mapa</button>
          </div>
        ) : (
          <div style={{ fontSize: 12, color: 'var(--gray-500)', marginTop: 4 }}>
            📍 {f.latitude.toFixed(6)}, {f.longitude.toFixed(6)}
            <button className="btn btn-small btn-secondary" style={{ marginLeft: 4 }} onClick={() => { setCoordInput(`${f.latitude}, ${f.longitude}`); }}>Editar</button>
            <button className="btn btn-small btn-secondary" style={{ marginLeft: 4 }} onClick={() => onStartPinMode(f.id)}>📍 Reposicionar</button>
          </div>
        )}
      </div>
      ...
      <div className="panel-section">
        ...
        {(f.membros || []).map((m) => {
          ...
          const detalhes = []
          if (m.papel_familia) detalhes.push(ROLE_LABELS[m.papel_familia] || m.papel_familia)
          ...
          return (
            <div key={m.id} className="membro-card">
              ...
              <div className="membro-info">
                <div className="membro-nome">{m.primeiro_nome} {m.sobrenome}</div>
                <div className="membro-detalhe">{detalhes.join(' • ')}</div>
                {(m.telefone || m.email) && (
                  <div style={{ marginTop: 3 }}>
                    {tel && <a href={`https://wa.me/${tel}`} target="_blank" rel="noreferrer" className="contact-link">📱 WhatsApp</a>}
                    {' '}
                    {m.email && <a href={`mailto:${m.email}`} className="contact-link">📧 {m.email}</a>}
                  </div>
                )}
              </div>
            </div>
      ...
      <div className="panel-section">
        ...
        <div className="form-nova-visita">
          <div className="form-row">
            <input type="date" value={visitData} onChange={(e) => setVisitData(e.target.value)} />
            <input type="text" value={visitVisitante} onChange={(e) => setVisitVisitante(e.target.value)} placeholder="Nome do visitante" />
          </div>
          <div className="form-row">
            <select value={visitTipo} onChange={(e) => setVisitTipo(e.target.value)}>
              <option value="visita">🚶 Visita</option>
              <option value="tentativa">🔔 Tentativa</option>
              <option value="ligacao">📞 Ligação</option>
              <option value="mensagem">💬 Mensagem</option>
            </select>
            <select value={visitResultado} onChange={(e) => setVisitResultado(e.target.value)}>
          ...
          <textarea value={visitNotas} onChange={(e) => setVisitNotas(e.target.value)} placeholder="Notas sobre a visita..." />
          <button className="btn btn-success" onClick={addVisita}>Registrar Visita</button>
        </div>
      </div>
      ...
      <div className="panel-section">
        ...
        {(f.visitas || []).length === 0 ? (
          <p style={{ color: 'var(--gray-400)', fontSize: 13, fontStyle: 'italic' }}>Nenhuma visita registrada ainda.</p>
        ) : (
          (f.visitas || []).map((v) => (
            <div key={v.id} className="visita-card">
              <button className="visita-delete" onClick={() => deleteVisita(v.id)} title="Remover">&times;</button>
              <div className="visita-data">
                📅 {formatDate(v.data_visita)}
                <span className="visita-tipo">{TIPO_LABELS[v.tipo] || v.tipo}</span>
                {v.resultado && (
              ...
              {v.notas && <div className="visita-notas">&quot;{v.notas}&quot;</div>}
            </div>
          ))
        )}
      </div>
