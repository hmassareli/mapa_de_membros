...
export default function MainPage() {
  ...
  // Filters
  const [filters, setFilters] = useState({ status: '', aceita_visitas: '', interesse_retorno: '' })
  ...
  const getFilterParams = useCallback(() => {
    ...
    if (filters.interesse_retorno) p.interesse_retorno = filters.interesse_retorno
    return p
  }, [filters])
  ...
  async function iniciarGeocodeBg() {
    try {
      ...
      setGeocodeProgress({ current: 0, total: fams.length, sucesso: 0, falha: 0 })

      geocodeClient.setOnProgress((p) => {
        setGeocodeProgress({ current: p.current, total: p.total, sucesso: p.sucesso, falha: p.falha })
      })
      geocodeClient.setOnComplete((p) => {
        showToast(`Geocodificação: ${p.sucesso} encontrados, ${p.falha} não encontrados`, 'success')
  ...
  async function iniciarRefinamentoNominatim() {
    ...
    try {
      ...
      setRefinamentoProgress({ current: 0, total: fams.length, sucesso: 0, falha: 0 })

      geocodeClient.setOnProgress((p) => {
        setRefinamentoProgress({ current: p.current, total: p.total, sucesso: p.sucesso, falha: p.falha })
      })
      geocodeClient.setOnComplete((p) => {
        showToast(`Refinamento: ${p.sucesso} melhorados, ${p.falha} sem resultado`, 'success')
  ...
  async function handleMapClickPin(lat, lng) {
    ...
    try {
      ...
      showToast(`📍 Localização salva para ${fam?.nome_familia || 'família'}!`, 'success')
      disablePinMode()
      refresh()
    } catch (err) {
  ...
  function flyToFamily(familia) {
    if (familia.latitude && familia.longitude && mapRef.current) {
      mapRef.current.flyTo([familia.latitude, familia.longitude], 18, { duration: 0.8 })
    }
  }
  ...
  const progressLabel = geocodeProgress ? '🗺️ Geocodificando...' : '🔄 Refinando coordenadas...'
        ...
        <div className="import-banner">
          ...
          <label className="import-banner-btn">
            ...
            <input
              ...
              accept=".json"
              style={{ display: 'none' }}
              onChange={(e) => e.target.files.length && handleImportFile(e.target.files[0])}
            />
          </label>
          <button className="import-banner-close" onClick={() => setImportNeeded(false)}>×</button>
        </div>
      )}
        ...
        {viewMode === 'map' ? (
          <>
            ...
            <FabGroup
              ...
              onShowNoCoords={() => {
                const noCoords = familias.filter((f) => !f.latitude || !f.longitude)
            ...
            {pinMode && (
              <div className="pin-mode-banner">
                <span>📍 Clique no mapa para marcar a localização. Selecione a família abaixo.</span>
                <select value={pinFamiliaId} onChange={(e) => setPinFamiliaId(e.target.value)}>
                  <option value="">-- Selecione uma família --</option>
                  <optgroup label="Sem coordenadas">
                    {familias.filter((f) => !f.latitude || !f.longitude).map((f) => (
                      <option key={f.id} value={f.id}>{f.nome_familia} - {f.endereco_linha1 || 'sem endereço'}</option>
                    ))}
                  </optgroup>
                  <optgroup label="Com coordenadas (reposicionar)">
                    {familias.filter((f) => f.latitude && f.longitude).map((f) => (
                      <option key={f.id} value={f.id}>{f.nome_familia}</option>
                    ))}
                  </optgroup>
                </select>
                <button className="btn btn-danger btn-small" onClick={disablePinMode}>Cancelar</button>
              </div>
            )}
      ...
      {progress && (
        <div className="geocode-bar">
          ...
          <span className="geocode-bar-text">
            {progress.current}/{progress.total} (✅{progress.sucesso} ❌{progress.falha})
          </span>
          <button
