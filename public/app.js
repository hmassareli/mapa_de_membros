...
function showImportBanner() {
  ...
  banner.style.cssText = 'position:fixed;top:0;left:0;right:0;background:linear-gradient(135deg,#f59e0b,#d97706);color:white;padding:12px 20px;text-align:center;z-index:9999;font-size:14px;font-family:Inter,sans-serif;display:flex;align-items:center;justify-content:center;gap:12px;';
  banner.innerHTML = `
...
async function uploadFromBanner(input) {
  ...
  try {
    ...
    if (res.ok) {
      showToast(`Importados: ${result.familias} fam\u00edlias e ${result.membros} membros!`, 'success');
...
async function iniciarGeocodeBg() {
  try {
    const res = await fetch(`${API}/api/geocodificar-batch`, { method: 'POST' });
    const data = await res.json();
    if (data.erro || !data.total) return;

    showGeocodeProgressBar(data.total);
  } catch (err) {}
}

async function checkGeocodeRunning() {
  try {
    const res = await fetch(`${API}/api/geocodificar-progresso`);
    if (!checkAuth(res)) return;
    const p = await res.json();
    if (p.running && p.total > 0) {
      showGeocodeProgressBar(p.total);
    }
  } catch (err) {}
}
...
function showGeocodeProgressBar(total) {
  ...
  bar.style.cssText = 'position:fixed;bottom:0;left:0;right:0;background:#1e293b;color:white;padding:8px 20px;z-index:9999;font-family:Inter,sans-serif;font-size:13px;display:flex;align-items:center;gap:12px;';
  bar.innerHTML = `
  ...
  const interval = setInterval(async () => {
    try {
      ...
      if (bgText) bgText.textContent = `${p.current}/${p.total} (✅${p.sucesso} ❌${p.falha})`;

      if (p.concluido || !p.running) {
        clearInterval(interval);
        showToast(`Geocodificação concluída: ${p.sucesso} encontrados, ${p.falha} não encontrados`, 'success');
...
function renderSidePanel(familia) {
  const membrosHtml = familia.membros.map(m => {
    const roleLabels = { HEAD: 'Chefe', SPOUSE: 'Cônjuge', CHILD: 'Filho(a)', OTHER: 'Outro' };
    const avatarClass = m.sexo === 'M' ? 'avatar-m' : 'avatar-f';
    const initials = (m.primeiro_nome || '?')[0].toUpperCase();

    let detalhes = [];
    if (m.papel_familia) detalhes.push(roleLabels[m.papel_familia] || m.papel_familia);
  ...
  const visitasHtml = (familia.visitas || []).map(v => {
    const tipoLabels = { visita: 'Visita', tentativa: 'Tentativa', ligacao: 'Ligação', mensagem: 'Mensagem' };
    const resultadoLabels = { atendeu: 'Atendeu', nao_atendeu: 'Não Atendeu', nao_estava: 'Não Estava', recusou: 'Recusou' };
  ...
  const coordSection = (!familia.latitude || !familia.longitude) ? `
...
function editCoordinates(familiaId) {
  ...
  const coordDisplay = document.querySelector('.panel-section .status-controls')?.parentElement;
...
async function onMapClickPin(e) {
  ...
  try {
    ...
    showToast(`📍 Localização salva para ${familia?.nome_familia || 'família'}!`, 'success');
...
function setupEventListeners() {
  ...
  document.getElementById('filtroVisitas').addEventListener('change', applyFilters);
  document.getElementById('filtroInteresse').addEventListener('change', applyFilters);
  ...
  // No coords panel
  document.getElementById('btnSemCoord').addEventListener('click', showNoCoordsPanel);
  document.getElementById('closeNoCoordsPanel').addEventListener('click', () => {
  ...
  document.getElementById('btnCancelPin').addEventListener('click', disablePinMode);
...
function showSearchDropdown(query) {
  ...
  const matches = familiasData.filter(f => {
  ...
  if (matches.length > 0) {
    ...
    matches.forEach(f => {
      const statusColors = {
        ativo: '#10b981', inativo: '#ef4444', nao_contatado: '#3b82f6',
        mudou: '#8b5cf6', desconhecido: '#6b7280'
      };
      const statusLabels = {
        ativo: 'Ativo', inativo: 'Inativo', nao_contatado: 'Não contatado',
        mudou: 'Mudou', desconhecido: 'Desconhecido'
      };
...
async function geocodeAndZoom(query) {
  ...
  try {
    ...
    geocodeMarker.bindPopup(`<strong>📍 Resultado da busca</strong><br><small>${display_name}</small>`).openPopup();
