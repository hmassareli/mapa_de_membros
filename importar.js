...
async function geocodificarTodas() {
  ...
  console.log(
    `   Tempo estimado: ~${Math.ceil(familiasSemCoord.length / 60)} minutos\n`,
  );
  ...
  for (let i = 0; i < familiasSemCoord.length; i++) {
    ...
    if (!coords && f.endereco_linha1 && f.endereco_linha3) {
      ...
      coords = await geocodificarEndereco(
        `${f.endereco_linha1}, ${f.endereco_linha3}`,
      );
    }
    ...
    if (!coords && f.endereco_linha1) {
      ...
      coords = await geocodificarEndereco(
        `${f.endereco_linha1}, São José dos Campos, SP, Brasil`,
      );
    }
    ...
    process.stdout.write(
      `\r   Progresso: ${progresso}% (${i + 1}/${familiasSemCoord.length}) | ✅ ${sucesso} | ❌ ${falha}`,
    );
...
rl.question(
  "\nDeseja geocodificar os endereços agora? (s/n): ",
  async (resposta) => {
    if (resposta.toLowerCase() === "s") {
      await geocodificarTodas();
    } else {
      console.log(
        "Você pode geocodificar depois executando: node geocodificar.js",
      );
    }
    rl.close();
    process.exit(0);
